...
```bash
git clone https://github.com/sarastee/avito-test-assignment
cd avito-test-assignment
```
...
## Тестирование API
Были написаны E2E тесты для аутентификации и получения баннеров для юзера.
Тесты так же запускаются в отдельных контейнера и схлопываются после выполнения.
